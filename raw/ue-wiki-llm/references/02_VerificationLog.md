...
2. **모듈별 한 줄 요약**. UE5 일반 지식 기반의 서술이며 5.7.4 트리에서 클래스/심볼이 그대로 존재하는지는 모듈별로 따로 확인해야 한다.
...
4. **버전별 차이**. `Engine/Source/Runtime/` 폴더 자체에는 5.7.4에서 신규 추가/이름변경된 모듈이 있을 수 있다 (`StateStream`, `MassEntity`, `CoreVerseVM` 등은 비교적 최신). 5.4/5.5와 비교 검증은 안 함.
...
2. **Tier 1**부터 모듈을 하나씩 골라 `skills/<Module>/SKILL.md` 를 작성한다. 형식 가이드:
   ...
   - 사용 예제 (가능하면 5.7.4 트리 안의 호출처를 인용)

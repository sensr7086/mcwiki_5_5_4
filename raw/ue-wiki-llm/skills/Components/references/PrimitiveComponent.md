...
| API | 라인 | 의미 |
|-----|------|------|
| `void IgnoreActorWhenMoving(AActor* Actor, bool bShouldIgnore)` | L1034 | 액터 무시 |
| `TArray<TObjectPtr<AActor>> CopyArrayOfMoveIgnoreActors()` | L1040 | 복사 |
| `void ClearMoveIgnoreActors()` | L1051 | 초기화 |
| `void IgnoreComponentWhenMoving(UPrimitiveComponent* Component, bool)` | L1069 | 컴포넌트 무시 |
| `void ClearMoveIgnoreComponents()` | L1086 | 초기화 |
| `virtual bool ShouldComponentIgnoreHitResult(FHitResult const& TestHit, EMoveComponentFlags MoveFlags)` | L1096 | override 가능 |
...
| `bool WasRecentlyRendered(float Tolerance=0.2f) const` | 최근 렌더? (L955) |

### 5.2 Material 동적 변경

| API | 라인 | 의미 |
|-----|------|------|
| `UMaterialInterface* GetMaterial(int32 ElementIndex) const` | L1109 | 가져오기 |
| `void SetMaterial(int32 ElementIndex, UMaterialInterface*)` | L1117 | 설정 |
| `int32 GetNumMaterials() const` | L1125 | 슬롯 수 |
| `UMaterialInstanceDynamic* CreateAndSetMaterialInstanceDynamic(int32 ElementIndex)` | L1133 | MID 생성 + 설정 |
| `UMaterialInstanceDynamic* CreateDynamicMaterialInstance(int32 ElementIndex, UMaterialInterface*=nullptr, FName OptionalName=NAME_None)` | L1137 | MID 생성만 |
| `UMaterialInstanceDynamic* GetCreateMaterialInstanceDynamic(int32 ElementIndex)` | L1141 | 가져오기 또는 생성 |
| `TArray<UMaterialInterface*> GetMaterials() const` | L1148 | 모든 머티리얼 |
| `void SetMaterialByName(FName MaterialSlotName, UMaterialInterface*)` | L1155 | 슬롯명으로 |
...
| API | 라인 | 의미 |
|-----|------|------|
| `bool IsExcludedFromHLODLevel(uint8 HLODLevel) const` | L728 | 특정 레벨 제외? |
| `void SetExcludedFromHLODLevel(uint8 HLODLevel, bool bSetExcluded)` | L732 | 설정 |

...
| 멤버 | 의미 |
|------|------|
| `EVisibilityBasedAnimTickOption VisibilityBasedAnimTickOption` (L737) | **가시성 기반 Tick 정책 (5종)** |
| `bool bEnableUpdateRateOptimizations:1` (L867) | URO 활성 |
| `bool bDisplayDebugUpdateRateOptimizations:1` (L873) | 디버그 표시 |
| `bool bComponentUseFixedSkelBounds:1` (L813) | 고정 바운드 (성능 + 컬링 정확도 ↓) |
| `bool bRenderStatic:1` (L879) | 정적 렌더 (애니 X) |
| `FAnimUpdateRateParameters* AnimUpdateRateParams` (L2122) | URO 파라미터 |
| `bool ShouldUseUpdateRateOptimizations() const` (L2128) | 사용 여부 |

### 7.2 🚨 EVisibilityBasedAnimTickOption — 5종 (SkinnedMeshComponent.h L93)
...
#### 7.4.1 FAnimUpdateRateParameters 구조 (`Engine/Classes/Engine/EngineTypes.h` L2472)

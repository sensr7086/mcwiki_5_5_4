...
| `bool IsBeingDestroyed() const` (L502) | 파괴 중? |
...
| API | 라인 | 의미 |
|-----|------|------|
| `void Activate(bool bReset=false)` | L563 | 활성 — 자식 자동 활성 |
| `void Deactivate()` | L569 | 비활성 |
| `void SetActive(bool bNewActive, bool bReset=false)` | L577 | 토글 |
| `void ToggleActive()` | L583 | |
| `bool IsActive() const` | (인라인) | |
| `void SetAutoActivate(bool)` | L597 | AutoActivate UPROPERTY 변경 |

### 4.3 소유자 / World

| API | 의미 |
|-----|------|
| `AActor* GetOwner() const` (L520) | 부모 액터 |
| `UWorld* GetWorld() const override final` (L531) | World 캐시 (성능) |
...
| `virtual bool ReplicateSubobjects(UActorChannel*, FOutBunch*, FReplicationFlags*)` | L631 | 서브오브젝트 복제 (수동) |
| `virtual ELifetimeCondition GetReplicationCondition() const` | L640 | 복제 조건 |
| `virtual void PreReplication(IRepChangedPropertyTracker&)` | L643 | 복제 직전 |
| `virtual bool GetComponentClassCanReplicate() const` | L646 | 클래스 자체 복제 가능? |

#### Iris (5.x 신규 복제 시스템)

| API | 라인 | 의미 |
|-----|------|------|
| `virtual void RegisterReplicationFragments(...)` | L748 | Iris 등록 |
| `virtual void OnReplicationStartedForIris(...)` | L755 | |
| `virtual void OnStopReplicationForIris(...)` | L761 | |
| `virtual void BeginReplication()` | L765 | |
| `virtual void EndReplication()` | L769 | |

### 4.6 에디터 콜백 🛠

| API | 의미 |
|-----|------|
| `virtual bool IsEditorOnly() const` (L691) | 에디터 빌드만? |
| `virtual void MarkAsEditorOnlySubobject()` (L694) | 에디터 전용 마킹 |
| `virtual TUniquePtr<FWorldPartitionComponentDesc> CreateClassComponentDesc() const` 🛠 (L438) | World Partition 디스크립터 |
...
| `virtual void OnRep_IsActive()` (L513) | IsActive 복제 변경 |
| `virtual void OnEndOfFrameUpdateDuringTick()` (L742) | 프레임 끝 갱신 |
| `virtual void OnPreEndOfFrameSync()` (L745) | 프레임 끝 동기 직전 |

### 5.3 활성화 정책

| 시그니처 | 의미 |
|----------|------|
| `virtual bool ShouldActivate() const` (L793) | 활성 가능? |
| `virtual bool ShouldAsyncCreatePhysicsState(UWorld*) const` (L796) | 비동기 물리 |
...
| `IsEditorOnly()` (L691) | `WITH_EDITORONLY_DATA` | |

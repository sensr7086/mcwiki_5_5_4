...
| 메소드 | 역할 | 라인 |
|--------|------|------|
| `FindFloor(CapsuleLoc, OutResult, bCanUseCachedLoc, DownwardSweepResult)` | 메인 진입점 | `L2071` |
| `ComputeFloorDist(CapsuleLoc, LineDist, SweepDist, OutResult, SweepRadius, DownwardSweepResult)` | 거리 계산 (실제 sweep) | `L2096` |
| `IsWalkable(Hit)` | Walkable 검사 — `WalkableFloorAngle` 비교 | `L1917` |
| `AdjustFloorHeight()` | 바닥과의 거리 조정 (떠 있지 않게) | `L1380` |
...
| 종류 | 필드 | 설명 |
|------|------|------|
| **Animation RootMotion** | `RootMotionParams` (`L2782`) | AnimMontage 의 RootBone 이동 — Server 권한 |
| **RootMotionSource** (Programmatic) | `CurrentRootMotion` (`L2727`) | 코드에서 만든 RootMotion (Force/Curve/Jump 등) — Server/Client 동일 시뮬 |

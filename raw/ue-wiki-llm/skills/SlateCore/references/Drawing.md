...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Rendering/DrawElements.h` | `class FSlateWindowElementList : FNoncopyable` (L219) | `OnPaint` 의 `OutDrawElements`. 한 윈도우의 draw element 누적. |
| `Rendering/DrawElementTypes.h` | `class FSlateDrawElement` (L45) | 단일 그리기 명령 (Box/Border/Text/Line/Spline/Gradient/Custom 등). 정적 `Make*` 헬퍼. |
...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `FastUpdate/SlateInvalidationRoot.h` | `class FSlateInvalidationRoot : FGCObject, FNoncopyable` (L76), `FSlateInvalidationContext` (L20), `FSlateInvalidationResult` (L60), `enum class ESlateInvalidationPaintType` (None/Slow/Fast) | 캐시 루트. `SInvalidationPanel`(→ `Slate` 모듈)이 이걸 감쌈. |

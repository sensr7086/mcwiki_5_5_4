...
| `FKeyEvent : FInputEvent` | L430 | 키보드 키 (`GetKey()`/`GetCharacter()`/`IsRepeat()`) |
| `FAnalogInputEvent` | L526 | 아날로그 (게임패드 스틱 등) |
| `FCharacterEvent` | L604 | 문자 입력 (IME 통합) |
| `FPointerEvent` | L693 | 마우스/터치 포인터 (`GetScreenSpacePosition()`/`GetEffectingButton()`/`GetWheelDelta()`/`GetTouchpadIndex()`) |
| `FMotionEvent` | L1058 | 모션 센서 (틸트·자이로) |
| `FNavigationEvent` | L1146 | UI 내비게이션 (방향/Accept/Cancel) |

### 2.3 라우팅·테스트 (`Public/Input/`)

| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Input/HittestGrid.h` | `class FHittestGrid : FNoncopyable` (L29) | 화면 좌표 → 위젯 경로 빠른 조회. 인밸리데이션 루트마다 보유. |

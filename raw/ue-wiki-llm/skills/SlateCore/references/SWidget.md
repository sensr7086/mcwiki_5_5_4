...
FSlateControlledConstruction
  └─ SWidget                       (Widgets/SWidget.h:162)
       ├─ SCompoundWidget          (SCompoundWidget.h:21)  ← 자식 1개 + ChildSlot
       ├─ SLeafWidget              (SLeafWidget.h:27)      ← 자식 없음 (STextBlock/SImage 부류)
       └─ SPanel                   (SPanel.h:28)           ← 자식 N개 + Slot 시스템
...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Public/Widgets/SWidget.h` | `class SWidget : FSlateControlledConstruction, TSharedFromThis<SWidget>` (L162) | 모든 Slate 위젯의 루트. `SLATE_DECLARE_WIDGET_API`. |
| `Public/Widgets/SCompoundWidget.h` | `class SCompoundWidget : SWidget` (L21) | 단일 자식 컨테이너. `ChildSlot` 패턴. UI 합성용 베이스. |
| `Public/Widgets/SLeafWidget.h` | `class SLeafWidget : SWidget` (L27) | 자식 없는 잎 위젯. `OnPaint`/`ComputeDesiredSize` 직접 구현. |
| `Public/Widgets/SPanel.h` | `class SPanel : SWidget` (L28) | 다중 자식 컨테이너. `OnArrangeChildren` 구현 필수. `FChildren* GetChildren()` 노출. |
| `Public/Widgets/DeclarativeSyntaxSupport.h` | `SLATE_BEGIN_ARGS` (L63), `SLATE_END_ARGS` (L116), `SLATE_ATTRIBUTE` (L194), `SLATE_ARGUMENT` (L210), `SLATE_NAMED_SLOT` (L439), `SLATE_DEFAULT_SLOT` (L446), `SLATE_EVENT` (L460) | 선언 매크로. |
...
| 시그니처 | 위치 | 의미 |
|----------|------|------|
| `virtual int32 OnPaint(...) const = 0` | SWidget.h L1650 | 그리기. `OutDrawElements` 에 `FSlateDrawElement::Make*` 추가. 다음 LayerId 반환. |
| `virtual void OnArrangeChildren(const FGeometry&, FArrangedChildren&) const = 0` | SWidget.h L1659 | 자식 배치 — Panel/CompoundWidget. |
| `virtual FVector2D ComputeDesiredSize(float LayoutScaleMultiplier) const = 0` | SWidget.h L731 | 위젯이 원하는 크기 — 부모 레이아웃 결정 입력. |
| `virtual FChildren* GetChildren() = 0` | SWidget.h L856 | 자식 리스트. Panel은 슬롯 컨테이너 반환. |
| `virtual FChildren* GetAllChildren()` | SWidget.h L864 | 시각적 자식 + 비-시각적 자식 (디버그용). |
| `virtual bool CustomPrepass(float LayoutScaleMultiplier)` | SWidget.h L710 | 커스텀 prepass 단계 — true 반환 시 기본 prepass 건너뜀. |
...
| 시그니처 | 위치 | 의미 |
|----------|------|------|
| `virtual bool ComputeVolatility() const` | SWidget.h L1616 | 매 프레임 재페인트 필요한지 — true 반환 시 캐시 비활성. |
| `virtual bool Advanced_IsInvalidationRoot() const` | SWidget.h L1524 | 자신이 인밸리데이션 루트인지 (`SInvalidationPanel`만 true). |
| `virtual const FSlateInvalidationRoot* Advanced_AsInvalidationRoot() const` | SWidget.h L1525 | 위 캐스트. |
| `virtual bool Advanced_IsWindow() const` | SWidget.h L1523 | `SWindow` 자식만 true. |

### 4.4 검증·접근성

`virtual bool IsInteractable() const` (L954), `virtual bool ValidatePathToChild(SWidget*)` (L689), `virtual FChildren* Debug_GetChildrenForReflector()` (L871).
...
| 항목 | 위치 | 가드 | 메모 |
|------|------|------|------|
| `Debug_GetChildrenForReflector()` 🛠 | SWidget.h L871 | 디버그 빌드 | Slate Reflector 위젯이 사용. |
| `IsInteractable()` 의 일부 경로 🛠 | SWidget.h L954 | 접근성 (Accessibility) 가드 | `Public/Widgets/Accessibility/` 안에 별도 헤더. |

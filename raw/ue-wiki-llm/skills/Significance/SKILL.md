...
| 헤더 | 클래스 | 의미 |
|------|--------|------|
| `SignificanceManager.h` L30 | `USignificanceManager` (UCLASS) | 글로벌 매니저 (UObject) — `USubsystem` 패턴 |
...
| `virtual void RegisterObject(UObject*, FName Tag, FManagedObjectSignificanceFunction, EPostSignificanceType=None, FManagedObjectPostSignificanceFunction=nullptr)` | L121 | 객체 등록 |
| `virtual void UnregisterObject(UObject*)` | L124 | 해제 |
...
| API | 의미 |
|-----|------|
| `virtual void Update(TArrayView<const FTransform> Viewpoints)` (L118) | 매 프레임 호출 — 모든 객체의 SignificanceFunction 호출 |

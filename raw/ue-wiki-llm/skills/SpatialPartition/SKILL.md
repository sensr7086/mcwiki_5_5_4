...
description: UE 5.7.4 공간 분할 카테고리 — 런타임 AActor 대규모 관리 + 반경 쿼리 (Octree / Quadtree / WorldPartition Runtime Spatial Hash / Static Spatial Index R-Tree). TOctree2 기반 AActor 트래커 표준 패턴 (가장 일반). 4 sub-skill 인덱스 + 결정 트리 + Semantics 작성 규약 + 함정. [SpatialPartition] prefix 호출.
---
...
**Engine 검증 사례** ([`PrimitiveSceneInfo.h:239`](../../Renderer-source) — `FScenePrimitiveOctree`):

...
> 🛠 위 70개 GraphEditor 헤더는 **본 위키 정식 분석 범위 외**(Editor 모듈). 라인 번호 인용은 신뢰성 한정 — 5.7.4 Editor 트리에서 확인된 것만 표기.
...
| `UEdGraphNode::GetNodeContextMenuActions`/`ValidateNodeDuringCompilation`/`GetMenuEntries` 🛠 | EdGraphNode.h L896~L967 | `WITH_EDITOR` | 에디터 콜백 다수. |

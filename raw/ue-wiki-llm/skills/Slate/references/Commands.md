...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Commands.h` | `template<typename CommandContextType> class TCommands : public FBindingContext` (L37) | **명령 묶음 베이스 (싱글턴)** — `Register()`/`Unregister()`/`Get()`/`IsRegistered()` 정적 + `RegisterCommands()` virtual 사용자 구현. |
| `Commands.h` | `UI_COMMAND(CommandId, Friendly, Desc, Type, Chord, ...)` (L23) | 명령 등록 매크로 — `LOCTEXT_NAMESPACE` 필수. |
| `Commands.h` | `UI_COMMAND_EXT(...)` (L21) | 확장 매크로 — namespace 직접 지정. |
| `Commands.h` | `UE_DECLARE_TCOMMANDS(Type, Api)` / `UE_DEFINE_TCOMMANDS(Type)` (L29-30) | DLL export 보장 매크로. |
| `Commands.h` | `MakeUICommand_InternalUseOnly(...)` (L17) | UI_COMMAND 매크로의 내부 호출 — 직접 호출 금지. |

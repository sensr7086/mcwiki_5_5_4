...
| 시그니처 | 위치 | 용도 |
|----------|------|------|
| `virtual void AllocateDefaultPins()` | EdGraphNode.h L704 | **핵심** — 노드 생성 시 핀 배열 초기화 (`CreatePin` 호출). |
| `virtual void ReconstructNode()` | L712 | 리프레시 시 핀 재구성 (옛 데이터 마이그레이션). |
| `virtual void PinDefaultValueChanged(UEdGraphPin*)` | L827 | 핀 기본값 변경 시 — 의존 노드 갱신. |
| `virtual void PinConnectionListChanged(UEdGraphPin*)` | L830 | 핀 연결/해제 시. |
| `virtual void PinTypeChanged(UEdGraphPin*)` | L833 | 핀 타입 변경 시 (Wildcard 등). |
| `virtual void NodeConnectionListChanged()` | L841 | 모든 핀 연결 변화 후 한 번. |
| `virtual void AutowireNewNode(UEdGraphPin* FromPin)` | L820 | 드래그로 노드 추가 시 자동 와이어. |
| `virtual void PostPlacedNewNode()` | L824 | 새 노드 배치 직후. |
| `virtual void PostPasteNode()` | L742 | 복붙 후. |
| `virtual void PrepareForCopying()` | L722 | 복사 전. |
| `virtual bool CanPasteHere(const UEdGraph*) const` | L727 | 붙여넣기 가능 여부. |
| `virtual bool CanCreateUnderSpecifiedSchema(const UEdGraphSchema*) const` | L732 | Schema 호환성. |
| `virtual TArray<UEdGraph*> GetSubGraphs() const` | L697 | 서브 그래프 (collapsed nodes). |
| `virtual void GetNodeContextMenuActions(UToolMenu*, UGraphNodeContextMenuContext*) const` 🛠 | L896 | 노드 우클릭 메뉴 항목. |
| `virtual void ValidateNodeDuringCompilation(FCompilerResultsLog&) const` 🛠 | L902 | 컴파일 검증. |
| `virtual void GetMenuEntries(FGraphContextMenuBuilder&) const` 🛠 | L908 | 그래프 메뉴 등록. |
| `virtual TSharedPtr<INameValidatorInterface> MakeNameValidator() const` 🛠 | L911 | 이름 검증기. |
| `virtual void OnRenameNode(const FString& NewName)` 🛠 | L914 | 이름 변경 콜백. |
| `virtual bool SupportsCommentBubble() const` 🛠 | L920 | 코멘트 버블 표시. |
| `virtual void OnPinRemoved(UEdGraphPin*)` | L926 | 핀 제거 시. |
| `virtual UEdGraphPin* GetPassThroughPin(const UEdGraphPin*) const` | L958 | 핀 통과(예: Knot 노드). |
| `virtual TSharedPtr<SGraphNode> CreateVisualWidget()` 🛠 | L964 | **사용자 정의 위젯 반환** — 핵심 override. |
| `virtual TSharedPtr<SWidget> CreateNodeImage() const` 🛠 | L967 | 노드 미니맵 이미지. |
...
| `virtual const FPinConnectionResponse CanCreateConnection(const UEdGraphPin* A, const UEdGraphPin* B) const` | L773 | 두 핀 연결 가능 여부 + 사용자 표시 메시지. |
| `virtual bool TryCreateConnection(UEdGraphPin*, UEdGraphPin*) const` | (베이스 자동) | 실제 연결 (LinkedTo 갱신). |
| `virtual const FPinConnectionResponse CanMergeNodes(const UEdGraphNode* A, const UEdGraphNode* B) const` | L786 | 두 노드 합치기 (Knot 등). |
| `virtual FLinearColor GetPinTypeColor(const FEdGraphPinType&) const` | L964 | 핀 색상 (FLinearColor). |
| `virtual FLinearColor GetPinColor(const UEdGraphPin*) const` | L974 | 개별 핀 색 (오버라이드). |
| `virtual FLinearColor GetSecondaryPinTypeColor(const FEdGraphPinType&) const` | L976 | 컨테이너 타입의 보조 색. |
| `virtual EGraphType GetGraphType(const UEdGraph*) const` | L1013 | GT_Function/GT_Macro/GT_Animation/GT_StateMachine. |
| `virtual bool IsTitleBarPin(const UEdGraphPin&) const` | L1022 | 타이틀바에 표시할 핀. |
| `virtual void SplitPin(UEdGraphPin*, bool bNotify=true) const` | L1051 | struct 핀 분해. |
| `virtual void RecombinePin(UEdGraphPin*) const` | L1054 | 분해된 핀 재결합. |
| `virtual void OnPinConnectionDoubleCicked(UEdGraphPin*, UEdGraphPin*, FVector2f) const` | L1059 | 와이어 더블클릭 (Reroute Knot). |
| `virtual bool IsSelfPin(const UEdGraphPin&) const` | L1080 | self 핀 판정. |
| `virtual void CreateDefaultNodesForGraph(UEdGraph&) const` | L1091 | 새 그래프 생성 시 기본 노드. |

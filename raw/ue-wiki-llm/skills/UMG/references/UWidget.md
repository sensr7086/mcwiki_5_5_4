...
| `Public/Components/Widget.h` | `class UWidget : public UVisual, public INotifyFieldValueChanged` (L216) | **모든 UMG 위젯의 베이스**. FieldNotification 인터페이스로 MVVM 통합. 80+ public 메서드 + 다수 virtual. |
...
| 멤버 | 위치 | 의미 |
|------|------|------|
| `UPROPERTY TObjectPtr<UPanelSlot> Slot` | Widget.h L283 | 부모 패널의 자식 배치 메타 (CanvasPanelSlot/HorizontalBoxSlot 등). UPROPERTY로 직접 디테일 노출. |
| `UPROPERTY FText ToolTipText` (L297) | Widget.h | 툴팁 — Setter `SetToolTipText` (L556) + `FieldNotify` (FieldNotification 자동). |
| `UPROPERTY TObjectPtr<UWidget> ToolTipWidget` | Widget.h | 커스텀 툴팁 위젯. |
| `UPROPERTY FWidgetTransform RenderTransform` (L297) | Widget.h | 렌더 변환 — Setter `SetRenderTransform`. |
| `UPROPERTY FVector2D RenderTransformPivot` (L305) | Widget.h | 변환 피벗. |
| `UPROPERTY ESlateVisibility Visibility` (L439) | Widget.h | 가시성 — Setter `SetVisibility` + `FieldNotify`. |
| `UPROPERTY bool bIsEnabled` (L326) | Widget.h | 활성 — Setter `SetIsEnabled` + `FieldNotify`. |
| `UPROPERTY float RenderOpacity` (L451) | Widget.h | 불투명도 — Setter `SetRenderOpacity`. |
| `uint8 bIsVolatile:1` | Widget.h L388 | 휘발성 (캐시 비활성). `ForceVolatile` (L620) 으로 토글. |

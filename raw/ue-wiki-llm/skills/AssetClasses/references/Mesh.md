...
description: UStaticMesh (2,543 lines) + USkeletalMesh (3,248) + USkeleton (1,037) + UPhysicsAsset (444) - 5.x Nanite + Compatible Skeleton + Virtual Bones + Ragdoll + LOD.
---

# AssetClasses/Mesh — UStaticMesh + USkeletalMesh + USkeleton + UPhysicsAsset

> **위치**: `Engine/Source/Runtime/Engine/Classes/Engine/StaticMesh.h` (2,543) + `SkeletalMesh.h` (3,248) + `Animation/Skeleton.h` (1,037) + `PhysicsEngine/PhysicsAsset.h` (444)
...
## 1. UStaticMesh (StaticMesh.h:592 — 2,543 lines)
...
{
    // StaticMesh.h:623 — 렌더 데이터 (LOD 별 Vertex/Index Buffer)
    TUniquePtr<class FStaticMeshRenderData> RenderData;

    // StaticMesh.h:735 — 5.x Nanite 설정
...
## 2. USkeletalMesh (SkeletalMesh.h:439 — 3,248 lines, 가장 큼)
...
## 3. USkeleton (Skeleton.h:294 — 1,037 lines)
...
## 4. UPhysicsAsset (PhysicsAsset.h:171 — 444 lines)
...
class UPhysicsAsset : public UObject,
                      ...
                      public IInterface_AssetUserData
{
    // PhysicsAsset.h:213 — Body 배열 (본 당 1개)
    UPROPERTY()
    TArray<TObjectPtr<USkeletalBodySetup>> SkeletalBodySetups;

    // PhysicsAsset.h:220 — Constraint 배열 (Body ↔ Body 연결)
    UPROPERTY()

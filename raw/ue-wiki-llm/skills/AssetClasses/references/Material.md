...
description: UMaterial (2,166) + UMaterialInstanceConstant + UMaterialInstanceDynamic + UMaterialInterface (1,385) - Domain 7종 + BlendMode 7종 + ShadingModel 12종 + 5.x PSO Precache.
---

# AssetClasses/Material — UMaterial + UMaterialInstance + UMaterialInterface

> **위치**: `Engine/Source/Runtime/Engine/Public/Materials/Material.h` (2,166) + `MaterialInstance.h` (1,256) + `MaterialInterface.h` (1,385) + `MaterialFunction.h` + `MaterialParameterCollection.h`
...
```
UObject
└── UMaterialInterface (1,385 lines — IBlendableInterface, IInterface_AssetUserData)
    ├── UMaterial (2,166 lines — 베이스 머티리얼, Material Editor 그래프)
    └── UMaterialInstance (1,256 lines — 파라미터 오버라이드 + Static Permutation)
...
## 2. UMaterial (베이스 머티리얼) — Material.h:431
...
{
    // Material.h:465 — 머티리얼 용도 결정
    UPROPERTY(EditAnywhere, Category=Material, AssetRegistrySearchable)
    TEnumAsByte<EMaterialDomain> MaterialDomain;

    // Material.h:469 — 블렌드 방식
    UPROPERTY(EditAnywhere, Category=Material)
    TEnumAsByte<EBlendMode> BlendMode;

    // Material.h:494 — Lighting Model
    UPROPERTY(EditAnywhere, Category=Material)
    TEnumAsByte<EMaterialShadingModel> ShadingModel;

    // Material.h:544 — 양면 렌더링
    UPROPERTY(EditAnywhere, Category=Material)
    uint8 TwoSided : 1;

    // Material.h:685 — Skeletal Mesh 사용 (Static Switch 활성)
    UPROPERTY()
    uint8 bUsedWithSkeletalMesh : 1;

    // Material.h:727 — Niagara Mesh 파티클
    UPROPERTY()
    uint8 bUsedWithNiagaraMeshParticles : 1;

    // Material.h:737 — Static Lighting (Lightmap)
    UPROPERTY()
...
## 3. UMaterialInstance (인스턴스 — 파라미터 오버라이드) — MaterialInstance.h:626
...
{
    // MaterialInstance.h:647 — 부모 Material (체인의 끝은 UMaterial)
    UPROPERTY(EditAnywhere)
    TObjectPtr<class UMaterialInterface> Parent;

    // MaterialInstance.h:610 — Static Parameters (셰이더 분기 — 컴파일 시 결정)

...
description: 🛠 Custom Material Expression 깊이 — UMaterialExpression 자손 작성 표준 + FMaterialCompiler API 600+ 메소드 + MIR::FEmitter 5.x Build 인터페이스 + UMaterialExpressionCustom 인라인 HLSL 노드 + Material Expression 269종 카테고리 + Substrate Material 5.x. Editor 전용 (WITH_EDITOR 가드 + 4단 분리).
---

# Render/MaterialExpression — Custom Material Expression 깊이 🛠

> **위치 (verified)**:
> - **UMaterialExpression** — `Engine/Source/Runtime/Engine/Public/Materials/MaterialExpression.h` (베이스, 라인 148)
> - **FMaterialCompiler** — `Engine/Source/Runtime/Engine/Public/MaterialCompiler.h` (674 virtual, 578 `int32` 리턴)
> - **UMaterialExpressionCustom** — `Engine/Source/Runtime/Engine/Public/Materials/MaterialExpressionCustom.h` (런타임 인라인 HLSL)
> - **MIR::FEmitter** — 5.x 신규 Material IR 빌더 (`MaterialExpression.h:35-37` namespace 선언)
> - **269 expressions** — `Engine/Public/Materials/MaterialExpression*.h` (Abs, Add, ..., VirtualTexture)
>
...
> 🚨 **현재 (5.7.4)** = `Compile()` 이 표준 / `Build()` = 실험적 (Substrate / Material IR 시스템). 일반 Custom Expression = `Compile()` 만 override.
...
| 10 | `Build(MIR::FEmitter&)` 실험적 인터페이스 의존 | 5.7.4 = `Compile()` 만 override (Build = 점진 전환) |

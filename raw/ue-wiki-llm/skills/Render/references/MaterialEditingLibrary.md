...
## 1. 58 UFUNCTION 카테고리 분류 [verified]
...
**총 58 UFUNCTION** — Material 자동화 거의 모든 영역.
...
| 항목 | 신뢰도 | 검증 출처 |
|------|--------|----------|
| UMaterialEditingLibrary 58 UFUNCTION | **[verified]** ✅ | `MaterialEditingLibrary.h` grep 58 매치 |
...
| 날짜 | 변경 |
|------|------|
| 2026-05-13 | 최초 작성. **UMaterialEditingLibrary 58 UFUNCTION [verified]** — 10 카테고리 분류 (Expression CRUD 8 / Connection 2 / Compile 4 / Layout 3 / Parameter Get 4 / MIC Get/Set 16 / Inspection 8 / Usage 2 / Parameter Names 7 / Group-Stats 4). 핵심 API 3 (`CreateMaterialExpression` / `ConnectMaterialProperty` / `ConnectMaterialExpressions`) 정확 시그니처 + 실 사용 예 + Python 자동화 + 시나리오 7종 + 함정 10. Engine 5.7.4 검증 — `Source/Editor/MaterialEditor/Public/MaterialEditingLibrary.h`. |

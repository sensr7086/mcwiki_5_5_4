...
description: UE 5.7.4 Plugin 카테고리 전문가 — GAS (UAbilitySystemComponent + UAttributeSet + UGameplayAbility + FGameplayEffect + FGameplayTag) + Niagara (UNiagaraComponent + 5.x VFX 표준) + Significance (USignificanceManager + 거리 LOD). MOBA / RPG / 다수 NPC / VFX 작업 시 호출. [GAS] / [Niagara] / [Significance] prefix 호출.

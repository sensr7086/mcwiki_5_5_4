...
---
name: animation-ik
description: UE 5.7.4 IK 시스템 — Legacy AnimNode IK + 5.x IK Rig + IK Retargeter. {keywords}
---
```
...
✅ 개선: APawn::PossessedBy(AController* NewController)
        // 5.7.4 기준 — 정확한 라인 = grep "PossessedBy" Source/Runtime/Engine/Classes/GameFramework/Pawn.h
```
...
**원리**: 모든 인용에 `@ue-5.7.4` 태그. 새 버전 = 자동으로 "재검증 필요" 플래그.

```markdown
> APawn::PossessedBy `@ue-5.7.4` — 라인 598
> @ue-5.8 검증 필요
```
...
**원리**: UE 5.7 → 5.8 git diff → 영향받는 SKILL.md 자동 식별.

```bash
git diff UE-5.7.4..UE-5.8.0 -- Engine/Source/Runtime/Engine/ | \
...
**실제** (UE 5.7.4 grep): AAT_None / AAT_LocalSpaceBase / AAT_RotationOffsetMeshSpace 3종

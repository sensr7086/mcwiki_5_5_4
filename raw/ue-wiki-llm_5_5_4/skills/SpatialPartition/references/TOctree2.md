...
description: TOctree2<T, Semantics> 기반 AActor 트래커 deep 표준 — Semantics 정의 + UWorldSubsystem 통합 + Register/Unregister/Update + GetActorsNearby (반경 쿼리 O(log N + K)) + 함정 12종. Engine FScenePrimitiveOctree (PrimitiveSceneInfo.h:259, UE 5.5) 검증 패턴 모사. 다수 AActor 매 프레임 반경 쿼리 표준 — AOE 스킬 / AI Sight / 군집 행동 / Influence Map.
---
...
>   - `Renderer/Public/PrimitiveSceneInfo.h:259, 828` — `FScenePrimitiveOctree` typedef + `FPrimitiveOctreeSemantics` (Renderer Visibility 베이스, 수만 Primitive 매 프레임 culling)
...
> **검증 출처**: `Renderer/Public/PrimitiveSceneInfo.h:259` typedef + `Renderer/Private/RendererScene.cpp` 실 호출 (5.5).

---

## 2. Semantics 정의 표준 [verified — FPrimitiveOctreeSemantics 모사]

`FPrimitiveOctreeSemantics` ([`PrimitiveSceneInfo.h:828-852`](../../Render/references/MeshDrawing.md), 5.5) 가 표준 — 모든 Semantics 가 따라야 할 6 가지 요소:
...
| 항목 | 신뢰도 | 검증 출처 |
|------|--------|----------|
| TOctree2 템플릿 시그니처 (생성자 / AddElement / RemoveElement / FindElementsWithBoundsTest) | **[verified]** ✅ | `Renderer/Public/PrimitiveSceneInfo.h:259` 사용 (5.5) + `Renderer/Private/LightSceneInfo.cpp` 실 호출 (FindElementsWithBoundsTest 매치) |
...
| FOctreeElementId2 SetElementId 패턴 | **[verified]** ✅ | `Renderer/Public/PrimitiveSceneInfo.h:847-851` (5.5, const_cast 패턴 동일) |

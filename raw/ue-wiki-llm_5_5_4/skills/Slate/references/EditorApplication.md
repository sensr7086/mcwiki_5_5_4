...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Public/Framework/Application/SlateApplication.h` | `class FSlateApplication : FSlateApplicationBase, FGenericApplicationMessageHandler` (L235) | **본체**. 두 베이스를 다중 상속 — `SlateApplicationBase` (Slate 측 추상) + `FGenericApplicationMessageHandler` (OS 메시지 수신). |
...
| `Public/Framework/Application/MenuStack.h` | `class FMenuStack` (L68), `struct FPopupTransitionEffect` (L30) | 팝업 메뉴 스택. |
...
FSlateApplication::Create();                                                  // L280
// 또는 GenericApplication 직접 주입:
FSlateApplication::Create(InPlatformApplication);                             // L281

FSlateApplication::InitializeCoreStyle();                                     // L287 — 기본 스타일셋 등록

// === B. Standalone 툴 (예: UnrealHeaderTool 같은 별도 exe) ===
TSharedRef<FSlateApplication> App =
    FSlateApplication::InitializeAsStandaloneApplication(MyRenderer);          // L283
// 또는 GenericApplication 도 직접:
TSharedRef<FSlateApplication> App =
    FSlateApplication::InitializeAsStandaloneApplication(MyRenderer, MyPlatformApp);  // L285

// === High-DPI 활성 ===
FSlateApplication::InitHighDPI(/*bForceEnable=*/false);                        // L318
// 메인 호출 전이어야 효과 — 보통 엔진 진입 직후

// === 종료 ===
FSlateApplication::Shutdown(/*bShutdownPlatform=*/true);                       // L312
```

### 3.2 접근

```cpp
if (FSlateApplication::IsInitialized())                                        // L294
{
    FSlateApplication& App = FSlateApplication::Get();                         // L305 — 메인 스레드 / Slate 스레드 / 비동기 로딩 스레드만 안전
    // ...
}
```
...
```
SlateApplication::Tick(TickType)                      // L380
   ↓
TickTime()                                            // L1181 — DeltaTime 갱신
   ↓
TickPlatform(DeltaTime)                               // L1186
   ↓ PumpMessages()                                   // L386 — OS 메시지 루프
   ↓ FGenericApplication::PollGameDeviceState
   ↓ MessageHandler 호출 → Process*Event(...)
   ↓
   ↓ IInputProcessor::Tick / Handle*Event              // 전역 후크 (위젯보다 먼저)
   ↓
TickAndDrawWidgets(DeltaTime)                         // L1191
...
FSlateApplication::Get().AddWindow(W, /*bShowImmediately=*/true);              // L1550

// === 모달 윈도우 (다른 입력 차단) ===
if (FSlateApplication::Get().CanAddModalWindow())                              // L389
{
    FSlateApplication::Get().AddModalWindow(W, ParentWidget,
                                             /*bSlowTaskWindow=*/false);       // L414
    // 모달 종료까지 블로킹
}

// === 부모 윈도우의 자식 (네이티브 차일드) ===
FSlateApplication::Get().AddWindowAsNativeChild(W, ParentWindow);              // L432
```
...
FSlateApplication::Get().RegisterInputPreProcessor(Proc);                      // L1482
// 또는 우선순위 인덱스 지정 (낮을수록 먼저)
FSlateApplication::Get().RegisterInputPreProcessor(Proc, /*Index=*/0);         // L1490
// 또는 타입별 (Pre / Post 등)
FSlateApplication::Get().RegisterInputPreProcessor(Proc, EInputPreProcessorType::Pre);  // L1498

// 해제 (모듈 셧다운에서)
FSlateApplication::Get().UnregisterInputPreProcessor(Proc);                    // L1512
```
...
```cpp
TSharedRef<FGlobalTabmanager> Tm = FSlateApplication::Get().GetGlobalTabManager();   // L315 (정적 별칭으로도 노출)

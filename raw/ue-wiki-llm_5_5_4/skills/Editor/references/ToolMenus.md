...
| `ToolMenuIteration.h` | 반복 헬퍼 — ⚠ **5.5 에는 없음 (5.7+ 추가)** | |
...
> **위치 주의**: UE 5.5 에서 `Engine/Source/Developer/ToolMenus/` (Developer). Editor 빌드에서 자동 활성화. 별개로 `ToolMenusEditor` 모듈 (Editor에 있음 — UnrealEd가 의존) 도 존재 — 추가 에디터 통합.

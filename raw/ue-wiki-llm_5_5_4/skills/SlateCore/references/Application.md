...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Application/SlateApplicationBase.h` | `class FSlateApplicationBase` (L103), `enum class EInvalidateWidgetReason` (L28 forward) | 추상 Application 베이스. 모든 `virtual = 0` 메서드는 `FSlateApplication` 이 구현. |
| `Application/SlateWindowHelper.h` | `FSlateWindowHelper` (정적 헬퍼) | 윈도우 정렬·검색 유틸. |
| `Application/ThrottleManager.h` | `class FThrottleRequest` (L13), `class FSlateThrottleManager` | 엔진 Tick 빈도를 일시적으로 낮추는 매니저 (드래그 중 등). |

...
> **베이스**: `UAnimInstance` (1,705 lines) + `FAnimInstanceProxy` (worker 스레드) + `FAnimNode_Base` (AnimGraph 노드) + `UAnimNotify` / `UAnimNotifyState`
...
| sub-skill | 책임 | 라인 | 핵심 |
|-----------|------|------|------|
| [`AnimInstance`](./AnimInstance/SKILL.md) ⭐ | UAnimInstance 라이프사이클 + Proxy + Curve + Montage_* | 1,705 | NativeUpdate/ThreadSafeUpdate 분리 / FAnimInstanceProxy / GetCurveValue |

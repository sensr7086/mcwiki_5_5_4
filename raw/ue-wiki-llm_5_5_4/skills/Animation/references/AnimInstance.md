...
description: UAnimInstance (1,705 lines) 라이프사이클 + FAnimInstanceProxy (worker 스레드) + Curve API + Montage_* 시리즈. NativeInitializeAnimation/NativeBeginPlay/NativeUpdateAnimation/NativeThreadSafeUpdateAnimation/NativeUninitializeAnimation 5단계 + Super 호출 규약 + 게임/워커 스레드 분리 표준.
---

# Animation/AnimInstance — UAnimInstance 라이프사이클 + Proxy + Curve + Montage

> **위치**: `Engine/Source/Runtime/Engine/Classes/Animation/AnimInstance.h` (1,705) + `Engine/Source/Runtime/Engine/Public/Animation/AnimInstanceProxy.h`
...
```cpp
// AnimInstance.h:1153 — Curve 값 조회

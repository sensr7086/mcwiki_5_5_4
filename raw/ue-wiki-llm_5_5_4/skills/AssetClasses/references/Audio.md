...
description: USoundBase (358) + USoundCue (367) + USoundWave (1,787) + USoundClass + USoundConcurrency + USoundMix + USoundAttenuation + 5.x MetaSounds.
---
...
> **파일**: `SoundBase.h` (358) + `SoundCue.h` (367) + `SoundWave.h` (1,787) + `SoundClass.h` + `SoundConcurrency.h` + `SoundMix.h` + `SoundAttenuation.h` + 5.x **MetaSoundSource** (Plugin)
...
```
UObject
└── USoundBase (358 — 베이스, IInterface_AssetUserData)
...
{
    // SoundBase.h:84 — Sound Class (Music / SFX / Voice / UI)
    UPROPERTY(EditAnywhere)
    TObjectPtr<USoundClass> SoundClassObject;

    // SoundBase.h:92 — Concurrency 오버라이드
    UPROPERTY(EditAnywhere)
    uint8 bOverrideConcurrency : 1;

    // SoundBase.h:149 — Concurrency 셋 (다중)
    UPROPERTY(EditAnywhere)
    TSet<TObjectPtr<USoundConcurrency>> ConcurrencySet;

    // SoundBase.h:163 — 길이 (초)
    UPROPERTY()
    float Duration;
...
## 4. USoundWave (음원 데이터 — 1,787 lines, 가장 큼)

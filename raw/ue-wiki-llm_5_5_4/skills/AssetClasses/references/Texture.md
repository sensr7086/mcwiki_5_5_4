...
description: UTexture (2,174) + UTexture2D + UTextureCube + UTextureRenderTarget2D + UVolumeTexture - CompressionSettings 10종 + TextureGroup 11종 + MipGenSettings 8종 + 5.x VirtualTexture/RVT.
---

# AssetClasses/Texture — UTexture + UTexture2D + UTextureCube + RenderTarget + VolumeTexture

> **위치**: `Engine/Source/Runtime/Engine/Classes/Engine/Texture.h` (2,174) + `Texture2D.h` (388) + `TextureCube.h` + `TextureRenderTarget2D.h` + `VolumeTexture.h`
...
└── UStreamableRenderAsset (Streaming 지원)
    └── UTexture (2,174 lines — 모든 Texture 베이스)
...
## 2. UTexture (베이스 — Texture.h:1220 — 2,174 lines)
...
{
    // Texture.h:1151 — 압축 설정 (디스크 저장 형식)
...
## 3. UTexture2D (Texture2D.h:21 — 가장 흔함)
...
{
    int32 GetSizeX() const;            // Texture2D.h:152

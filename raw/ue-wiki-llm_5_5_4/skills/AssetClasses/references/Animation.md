...
description: UAnimSequence (973) + UAnimMontage (982) + UBlendSpace (948) + UAnimBlueprint (299) + UAnimInstance (1,705) - 5.x NativeThreadSafeUpdateAnimation + Curve API + Montage_*.
---
...
> **파일**: `AnimSequence.h` (973) + `AnimSequenceBase.h` (351) + `AnimMontage.h` (982) + `BlendSpace.h` (948) + `AnimBlueprint.h` (299) + `AnimInstance.h` (1,705) + Skeleton (이미 [`AssetClasses/Mesh §3`](../Mesh/SKILL.md))
...
└── UAnimBlueprint (299 lines — TargetSkeleton 페어)
    ↓ 컴파일
    ├── 일반 클래스: UAnimInstance (1,705 lines)
...
## 2. UAnimSequence (가장 흔함 — 973 lines)
...
{
    // AnimSequence.h:308 — Additive 종류 (베이스 위 더하기)
    UPROPERTY()
    TEnumAsByte<enum EAdditiveAnimationType> AdditiveAnimType;

    // AnimSequence.h:320 — Additive 베이스 포즈 (Ref Pose / 다른 시퀀스 / 첫 프레임)
    UPROPERTY()
    TObjectPtr<class UAnimSequence> RefPoseSeq;

    // AnimSequence.h:301 — 압축 데이터 (Cooked + DDC)
...
```cpp
// AnimSequenceBase.h:217 — 시간 / 본 → Pose
...
## 3. UAnimMontage (Section + Notify + RootMotion — 982 lines)
...
{
    // AnimMontage.h:628 — BlendIn 설정
    UPROPERTY(EditAnywhere)
    FAlphaBlend BlendIn;

    // AnimMontage.h:637 — BlendOut 설정
    UPROPERTY(EditAnywhere)
...
```cpp
// AnimMontage.h:187-204 — 4종 Delegate
...
## 4. UBlendSpace (948 lines — 1D / 2D Blend)
...
## 6. UAnimInstance (런타임 — 1,705 lines, 가장 큼)
...
{
    // AnimInstance.h:1320 — 초기화 (BeginPlay 시점)
...
```cpp
// AnimInstance.h:1153 — Curve 값 조회 (AnimSequence / Montage 의 Curve)

---
name: input-action
description: UInputAction (ValueType 4종 - Bool/Axis1D/Axis2D/Axis3D) + ETriggerEvent 5종 (Triggered/Started/Ongoing/Canceled/Completed + None) + UInputTrigger 10종 + UInputModifier 11종 (UE 5.5).
---
...
| `UInputTriggerChordAction` | **다른 Action 과 함께 눌림** | `ChordAction` (참조) |
| `UInputTriggerChordBlocker` | Chord 의 inverted — Chord 활성 시 차단 | `ChordAction` |
| `UInputTriggerCombo` | **시퀀스 (예: 위→위→아래→발사)** | `Combo` 배열 |
...
IA_Sprint = UInputAction (Bool)
  - Triggers:
    - UInputTriggerChordAction (ChordAction = IA_ShiftKey)   // Shift 도 눌려있어야
```
...
## 4. UInputModifier 11종 (변환)

> **입력 값 변환** — 매핑된 키의 raw 값 → 게임 로직이 사용할 값. 여러 Modifier = 순차 적용. UE 5.5 concrete 클래스 11종.
...
| `UInputModifierScalar` | **3축 배율 (FVector)** — 5.5 에선 Scalar 가 유일 (구 Scale 통합) | `Scalar` (FVector) |
| `UInputModifierScaleByDeltaTime` | DeltaTime 적용 | (없음) |
...
| `UInputModifierSmoothDelta` | DeltaTime 기반 smoothing | `Smoothing*` |
...
| `UInputModifierToWorldSpace` | 로컬→월드 변환 | (없음) |
...
IA_Move (Axis2D)
  Modifiers:
    ...
    [1] Scalar (1.5, 1.5, 0)         // 2단계: 1.5 배 증가 (UE 5.5 — Scale 은 Scalar 로 통합)

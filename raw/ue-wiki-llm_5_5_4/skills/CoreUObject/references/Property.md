...
| `Public/UObject/UnrealType.h` / `Class.h` | `UPropertyWrapper`, `UMulticastDelegatePropertyWrapper`, `UMulticastInlineDelegatePropertyWrapper` | FProperty를 UObject로 감싸 디테일 패널/스크립트에서 사용 가능하게. **에디터 그리드 위주**. (5.5 에서는 별도 `PropertyWrapper.h` 없음 — `UnrealType.h` / `Class.h` 안 정의) |

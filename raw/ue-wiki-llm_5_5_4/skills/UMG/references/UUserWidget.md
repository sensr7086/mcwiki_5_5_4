...
| 헤더 | 심볼 | 역할 |
|------|------|------|
| `Blueprint/UserWidget.h` | `class UUserWidget : public UWidget, public INamedSlotInterface` (L275, UE 5.5) | **BP 위젯의 베이스**. 친구 `SObjectWidget`. UObject 라이프사이클 + Native\* virtual + Initialize + AddToViewport/PlayerScreen + ColorAndOpacity/ForegroundColor/Padding/InputActionPriority/IsFocusable/InputActionBlocking 등 BP 노출 멤버. |
| `Blueprint/UserWidget.h` | `enum class EWidgetTickFrequency : uint8` (L108, UE 5.5) | **Never** (절대 tick 안 함) / **Auto** (BP tick 함수 / latent action / 애니메이션 / 비-UserWidget 부모 native tick 시 자동). `meta=(DisableNativeTick)` 클래스 메타로 native tick 비활성. |
| `Blueprint/UserWidget.h` | `enum class EWidgetAnimationEvent : uint8` (Started/Finished, L123) | UWidgetAnimation 시작/종료 이벤트. |
...
| 멤버 | 위치 | 의미 |
|------|------|------|
| `EWidgetTickFrequency TickFrequency` | UserWidget.h L1649 (5.5) | 기본 `Auto` — `Never` 권장 (성능). |
...
| `FMargin Padding` (L985, 5.5) | UserWidget.h | 위젯 외곽 padding. |
| `int32 InputActionPriority` (L988 UPROPERTY, 5.5) | UserWidget.h | 입력 액션 우선순위. (Setter L1631) |
| `bool bIsFocusable` (L994 bitfield, 5.5) | UserWidget.h | 키보드 포커스 받을 수 있는지. UE_DEPRECATED 5.2 (Getter 사용). |
| `bool bIsInputActionBlocking` | UserWidget.h (5.5 — UPROPERTY 가시 위치 변경됨, grep 으로 확인 후 사용) | 입력 액션 차단. |
...
HUD->AddToViewport(/*ZOrder=*/0);                                         // L334 (5.5) — 전체 화면
HUD->AddToPlayerScreen(/*ZOrder=*/10);                                    // L343 (5.5) — 스플릿스크린의 한 플레이어 영역만

// === 제거 ===
HUD->RemoveFromParent();                                                  // 5.1+ 권장
// HUD->RemoveFromViewport();                                             // L350 (5.5) — DEPRECATED 5.1

// === 플레이어 컨텍스트 ===
HUD->SetOwningPlayer(MyPlayerController);                                 // L439 (5.5)
HUD->SetPlayerContext(FLocalPlayerContext(MyLocalPlayer));                // L391 (5.5)
APlayerController* PC = HUD->GetOwningPlayer();                           // L422 (5.5)
ULocalPlayer* LP      = HUD->GetOwningLocalPlayer();                      // L400 (5.5)
const FLocalPlayerContext& Ctx = HUD->GetPlayerContext();                 // L394 (5.5)
```
...
EWidgetTickFrequency Cur = MyWidget->GetDesiredTickFrequency();           // L294 (5.5)

...
> ⚠ **UE 5.5 line 번호 주의**: 본 §1 표의 `L####` 모두 UE 5.7.4 기준 — UE 5.5.4 에서는 평균 ~63 라인 위로 이동. 5.5 핵심 위치: `NativeOnInitialized` L1511 / `NativePreConstruct` L1512 / `NativeConstruct` L1513 / `NativeDestruct` L1514 / `NativeTick` L1515 / `NativePaint` L1521. 다른 Native\* 핸들러는 `grep -n "NativeOn함수명" Source/Runtime/UMG/Public/Blueprint/UserWidget.h` 로 즉시 검증.

### 1.1 라이프사이클 (5 — 가장 자주 override)

| 시그니처 | 위치 (5.5 / 5.7) | 호출 시점 / 용도 |
|----------|------------------|------------------|
| `virtual void NativeOnInitialized()` | UserWidget.h L1511 / L1574 | **`Initialize()` 안에서 한 번만** — 외부 시스템 구독·초기 데이터 로드. PIE/standalone 모두 동작. |
| `virtual void NativePreConstruct()` | UserWidget.h L1512 / L1575 | 디자이너 미리보기·실행 전 한 번. **디자이너 변경 즉시 반영**되는 hook. |
| `virtual void NativeConstruct()` | UserWidget.h L1513 / L1576 | **뷰포트 추가될 때마다** — 데이터 새로고침·델리게이트 바인딩. |
| `virtual void NativeDestruct()` | UserWidget.h L1514 / L1577 | 뷰포트에서 제거될 때 — 델리게이트 언바인딩·외부 참조 해제. |
| `virtual void NativeTick(const FGeometry&, float DeltaTime)` | UserWidget.h L1515 / L1578 | **매 프레임** (TickFrequency 결정 시) — **최대한 회피**. ActiveTimer / 이벤트 기반으로 대체 권장. |

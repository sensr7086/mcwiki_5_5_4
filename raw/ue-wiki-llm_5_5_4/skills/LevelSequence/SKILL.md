---
name: levelsequence
description: UE 5.5.4 LevelSequence 카테고리 — 시네마틱 / 컷씬 / 영상 시스템. 10 sub-skill (MovieScene 베이스 + Tracks 43종 + LevelSequencePlayer + Director + CineCamera + Sequencer 🛠 + MovieRenderPipeline 5.x + EntitySystem ECS + SequencerScripting + TemplateSequence). 런타임 컷씬 재생 + Sequencer Editor 확장 + Movie Render Queue + BP Director Event Track + UCineCameraComponent + 5.x ECS 평가. [LevelSequence] prefix 호출.
---

# LevelSequence — 시네마틱 / 컷씬 / 영상 시스템

> **위치 (verified)**:
> - **MovieScene** (Runtime 베이스): `Engine/Source/Runtime/MovieScene/` (UE 5.5 — 55 Public 헤더 + EntitySystem 64 + Evaluation 39 + Channels 22)
> - **MovieSceneTracks** (빌트인 트랙): `Engine/Source/Runtime/MovieSceneTracks/` (UE 5.5 — Tracks 41 + Sections + Systems, 총 186 헤더)
> - **LevelSequence** Runtime 모듈: `Engine/Source/Runtime/LevelSequence/Public/` (UE 5.5 — 16 헤더 — `ULevelSequence` / `ALevelSequenceActor` / `ULevelSequencePlayer` / `ULevelSequenceDirector`)
> - **CinematicCamera**: `Engine/Source/Runtime/CinematicCamera/Public/` (UE 5.5 — 6 헤더 — `UCineCameraComponent` / `ACineCameraActor` / `UCineCameraSettings` / `ACameraRig_Crane` / `ACameraRig_Rail`)
